...
//CHECK IF USER ALREADY SEND AN APPLICATION
if ($_USER->logged_in) {
    $DB->execute("SELECT username FROM applications WHERE username = :USERNAME", false, [":USERNAME" => $_USER->username]);
    if ($DB->RowNum == 1) {
...
if (isset($_POST["submit"]) && $_USER->logged_in && !$Application_Sent) {
    $_GUMP->validation_rules(array(
        "f_name"        => "required|max_len,100",
        "l_name"        => "required|max_len,100",
        "what"          => "required|max_len,500",
        "why"           => "required|max_len,500",
        "day"           => "is_day",
        "month"         => "is_month",
        "year"          => "is_year",
        "country"       => "required"
    ...
    if ($Validation) {
        ...
        $Name       = $Validation["f_name"]." ".$Validation["l_name"];
        $What       = $Validation["what"];
        $Why        = $Validation["why"];
        $Country    = $Validation["country"];
        $Birthday   = $Validation["year"]."-".$Validation["month"]."-".$Validation["day"];

        if (array_key_exists($Country,$Countries)) {
            $Insert = $DB->modify("INSERT INTO applications VALUES(:USERNAME,:NAME,:BIRTHDAY,:COUNTRY,:WHAT,:WHY,NOW(),0,0)",
                                 [
            ...
            notification("You successfully applied for partner. You will be notified once your application has been reviewed!","/partners","green"); exit();
        }
    } else {
        print_r($_GUMP->get_errors_array());
    }
}
...
if (!isset($_GET["signup"])) {
    ...
    $Info = $DB->execute("SELECT username, email FROM users WHERE username = :USERNAME", true, [":USERNAME" => $_USER->username]);
} else {
    redirect("/");
}
