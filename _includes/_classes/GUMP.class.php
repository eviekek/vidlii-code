...
GUMP::add_validator("equals", function($field, $input, $param = NULL) {
    if ($input[$field] === $input[$param]) {
        return true;
    } else {
        return false;
...
GUMP::add_validator("NoHTML", function($field, $input, $param = NULL) {
    if (!strpos($input[$field],"<") and !strpos($input[$field],">")) {
        return true;
    } else {
        return false;
    }
...
GUMP::add_validator("is_day", function($field, $input, $param = NULL) {
    if ($input[$field] <= 31 and $input[$field] > 0) {
        return true;
    }
});

GUMP::add_validator("is_month", function($field, $input, $param = NULL) {
    if ($input[$field] <= 12 and $input[$field] > 0) {
        return true;
    }
});

GUMP::add_validator("is_year", function($field, $input, $param = NULL) {
    if ($input[$field] <= date("Y") and $input[$field] > 1909) {
        return true;
    }
});

GUMP::add_filter("NoHTML", function($value, $params = NULL) {
    return str_replace("<","&lt;",str_replace(">","&gt;",$value));
});

GUMP::add_validator("is_hex", function($field, $input, $param = NULL) {
    if (strlen($input[$field]) == 7) {
        if (strpos($input[$field],"#") === 0) {
            return true;
        } else {
            return false;
        }
