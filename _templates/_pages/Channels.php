...
<div class="vc_r">
    <div class="vc_hd">
        <ul>
            <? foreach ($Header as $value => $item) : ?><? if ($value !== $Current_Order) : ?><li><a href="/channels?c=<?= $Current_Cat ?>&o=<?= $value ?>&t=<?= $Current_time ?>"><?= $item ?></a></li><? else : ?><li id="vc_selec"><?= $item ?></li><li style="padding:0 45px"></li><? endif ?><? endforeach ?>
        </ul>
    </div>
